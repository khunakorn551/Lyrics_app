<x-app-layout>
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex space-x-2">
                                                <button onclick="openModal('{{ $request->id }}')" class="text-indigo-600 hover:text-indigo-900">Update Status</button>
                                                <form action="{{ route('song-requests.destroy', $request) }}" method="POST" class="inline">
                                                    @csrf
                                                    @method('DELETE')
    ...
    <div id="statusModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full">
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div class="mt-3">
                <h3 class="text-lg font-medium leading-6 text-gray-900 mb-4">Update Request Status</h3>
                <form id="statusForm" method="POST" class="space-y-4">
                    ...
                    <div>
                        <label for="admin_response" class="block text-sm font-medium text-gray-700">Response</label>
                        <textarea id="admin_response" name="admin_response" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"></textarea>
                    </div>
    ...
    <script>
        function openModal(requestId) {
            ...
            form.action = `/song-requests/${requestId}`;
